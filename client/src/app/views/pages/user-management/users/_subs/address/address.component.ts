...
export class AddressComponent implements OnInit {
	...
	@Input() citizenship: BehaviorSubject<string>;
	@Input() rediance: BehaviorSubject<string>;
	hasFormErrors = false;
	...
	if (!this.citizenship.value) {
			const newAddress = ''
			this.citizenship.next(newAddress);
		}
		if (!this.rediance.value) {
			const newAddress = ''
			this.rediance.next(newAddress);
		}
		this.createForm();
	...
	createForm() {
		...
		// });
		this.addressForm = this.fb.group({
			countryCitizenship: [this.citizenship.value],
			// city: [this.addressSubject.value.city, Validators.required],
			// state: [this.addressSubject.value.state, Validators.required],
			countryResidence: [this.rediance.value]
		});
	}
	...
	updateAddress() {
		...
		// this.addressSubject.next(newAddress);
		this.citizenship.next(controls.countryCitizenship.value);
		this.rediance.next(controls.countryResidence.value);
		// this.addressSubject.next(newAddress);
	}
