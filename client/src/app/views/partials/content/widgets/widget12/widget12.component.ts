...
export class Widget12Component implements OnInit {
	...
	ngOnInit(): void {
		if (!this.data) {
			...
			console.log('====== widget 12 =======')
			console.log(this.userArr)
			for(let i = 1; i<= nowDate.getDate();i++){
				dataLabel[i-1] = `${i} ${MonthLabel[nowDate.getUTCMonth()]}`;
				dataNum[i - 1] = 0;
				var tempUser = this.userArr.filter(element => {
					var tempDate :Date = new Date(element.date);
					// console.log(tempDate + ' = ' + tempDate.getDay() + ' = ' + i)
					// console.log(tempDate.getDate() + ' + ' + tempDate.getFullYear() + ' + ' +tempDate.getMonth() + ' + ' +tempDate.getDay() + ' + ');
					return tempDate.getMonth() == nowDate.getMonth() && tempDate.getDate()  == i
				})
				
				if(tempUser){
					dataNum[i - 1] = tempUser.length;
				}
			}
			this.data = {
				labels: dataLabel,
